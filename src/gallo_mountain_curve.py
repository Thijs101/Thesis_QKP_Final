...
import numpy as np
import pandas as pd
from Dynamic_Programming_QKP import generate_gallo_instance, fl_heuristic_enhanced, solve_qkp_gurobi
...
def run_experiment_Gallo(n_val, density, num_instances=30, beam_width=1, gurobi_time_limit=600):
    """
    Solves a set of Gallo problems (heavy weights) and finds the average profit and time.
    """
    fl_times = []
    fl_values = []
    instance_gaps = []
    
    # Limit Gurobi execution to smaller instances (N <= 50) due to exponential time scaling
    use_gurobi = (n_val <= 50)
    
    for instance_num in range(num_instances):
        seed = instance_num + n_val * 1000 + int(density * 100)
        n_inst, capacity, Q, weights = generate_gallo_instance(n_val, density, seed=seed)
...
def collect_gallo_curve_data(n_val, density, beam_steps, num_instances=30, gurobi_time_limit=600, csv_file="data/gallo_mountain_curve.csv"):
    """
    This is the main function called by the benchmark suite to get Gallo data.
    """
    ...
    for b in beam_steps:
        ...
        df_temp = run_experiment_Gallo(n_val, density, num_instances, b, gurobi_time_limit)
